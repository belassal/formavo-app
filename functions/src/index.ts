...
export const seedDemoClub = functions.https.onRequest(async (req, res) => {
  if (req.query.key !== SEED_KEY) { res.status(403).send('forbidden'); return; }
  const ownerEmail = String(req.query.ownerEmail || '');
  if (!ownerEmail) { res.status(400).send('ownerEmail required'); return; }
  ...
  const pName = (i: number) => PLAYERS[i][0];
  const pId = (i: number) => playerIds[i];

  // ── Completed matches (goals: [scorerIdx, assistIdx|null, minute]) ──
  const COMPLETED: {
    date: string; opponent: string; home: number; away: number;
    comp: string; compName?: string;
    goals: [number, number | null, number][];
    card?: number;
  }[] = [
  ...
  for (const m of COMPLETED) {
    ...
    for (let i = 0; i < PLAYERS.length; i++) {
      batch.set(matchRef.collection('roster').doc(pId(i)), {
        playerId: pId(i), playerName: pName(i),
        role: i < 9 ? 'starter' : 'bench', attendance: 'present',
        ...(i < 9 ? { slotKey: SLOTS[i] } : {}),
        rsvpStatus: 'attending', createdAt: now, updatedAt: now,
      });
    }
    for (const [scorer, assist, minute] of m.goals) {
      batch.set(matchRef.collection('events').doc(), {
        type: 'goal', side: 'home', minute,
        scorerId: pId(scorer), scorerName: pName(scorer),
        ...(assist != null ? { assistId: pId(assist), assistName: pName(assist) } : {}),
        pos: { x: 0.3 + (minute % 5) * 0.1, y: 0.78 + (minute % 3) * 0.05 },
        createdAt: now,
      });
    }
  ...
  for (const m of UPCOMING) {
    ...
    await matchRef.set({
      ...
      dateISO: m.date, location: 'Demo Park, Bedford', fieldName: '',
      format: '9v9', formation: '3-3-2', halfDuration: 30,
      status: 'scheduled', competitionType: m.comp, competitionName: (m as any).compName || '',
  ...
  for (const t of trainings) {
    await teamRef.collection('trainings').doc().set({
      ...
      confirmedPlayerIds: (t as any).confirmed || [], declinedPlayerIds: (t as any).declined || [],
      attendedPlayerIds: (t as any).attended || [], createdAt: now, updatedAt: now,
    });
  }
